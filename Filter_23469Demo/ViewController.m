...
//  Filter_23469Demo
//
//  Created by 鲸鱼集团技术部 on 2020/8/10.
...
- (void)filterInit {
    ...
    //7.设置视口
    glViewport(0, 0, [self getDrawableWidth], [self getDrawableHeight]);

    //8.设置顶点缓存区
    GLuint vertexBuffer;
    glGenBuffers(1, &vertexBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
    GLsizeiptr bufferSizeButes = sizeof(SenceVertex) * 4;
    glBufferData(GL_ARRAY_BUFFER, bufferSizeButes, self.vertices, GL_STATIC_DRAW);

    //9.设置默认着色器
    [self setupNormarShaderProgram];
...
- (void)bindRenderLayer:(CAEAGLLayer *)layer {
    ...
    //2.获取帧缓存区名称，绑定渲染缓存区以及将渲染缓存区与layer建立连接
